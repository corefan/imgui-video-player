...
Framebuffer::Framebuffer(size_t w, size_t h, Eigen::VectorXf* v)
: _real_vertices{v}, _w{w} , _h{h}, _camera{0.0f, 0.0f, 20.0f}, _cameraTarget{0.0f, 0.0f, 0.0f},
	_up{0.0f,1.0f,0.0f}, _near{0.0001f}, _far{100.0f}, _fov{120.0f}
{
...
void Framebuffer::SetVertices(Eigen::VectorXf* v)
{
	_real_vertices = v;
}
...
void Framebuffer::Render()
{
	...
	std::cout << "framebuffer sees " << _real_vertices->size() << " vertices" << std::endl;
	glBindVertexArray(_vertices_buf);
	glDrawArrays(GL_POINTS, 0, (_real_vertices->size())/3 );
